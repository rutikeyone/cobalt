...
| **Декораторы** | обёртка вокруг того, что отдаёт регистрация, — логирование, повторы, кэш — без правки её класса, руками или через `@CobaltDecorates` |
...
| **Плагин линтера** | шестнадцать правил на том же слое разбора, которым пользуется генератор |
...
`cobalt_lint` — это `analysis_server_plugin`, а не плагин `custom_lint`. В нём шестнадцать
...
скоупы, а не посмотреть на `notes_app`. Шестнадцать записей в шести секциях:

| Секция | Записи |
|---|---|
| Старт | Двухфазный старт · Окружения · Ленивый async |
| Инъекция | Property injection · Именованные и мульти-инъекции · Декораторы |
