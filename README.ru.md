...
Поэтому три инструментальных пакета объявляют `analyzer: ">=10.0.1 <13.0.0"`, а не одну версию. Это
диапазон ровно из двух рабочих строк, и каждый пакет, читающий анализатор, пиннит его точно — так
что какую строку вы получите, решает ваш проект, а не мы:
...
| что-то новее | 12.1.0 | 0.14.8 | 0.3.14 | 0.2.5 | 3.1.8 |

Обе версии `dart_style` дают для сгенерированного кода одинаковые байты — 3.1.7 это bump
зависимости, а изменения стиля в 3.1.8 версионированы языком выше этого пола, — поэтому один и тот же
исходник порождает один и тот же файл на обеих строках. Это проверяется, а не предполагается: job
пола пересобирает на 3.38.9 и сверяет с закоммиченным по разу на строку — стенд совместимости для
новой и `codegen_basics`, Flutter-пакет, для старой.
