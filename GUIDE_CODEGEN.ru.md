...
Чтобы он был готов до открытия экрана, прогрейте его: `CobaltAppScope(warmUp: [CobaltKey(SearchEngine)])`
запускает сборку сразу после подъёма графа, за приложением, а не за `loading`, а `scope.warmUp([...])`
...
### Обёртка вокруг регистрации

`@CobaltDecorates` оборачивает то, что отдаёт регистрация, не трогая её класс, — логирование,
повторы, кэш, метрика вокруг клиента из чужого пакета:
...
Сам класс не регистрируется. Генератор пишет вокруг него `CobaltDecorator` и
`scope.decorate<ApiClient>(...)` в `build()`, поэтому каждый `get<ApiClient>()`, включая
внедряемые поля, получает обёртку. В `examples/codegen_basics` такой декоратор стоит на `Repository`.

Правила, у каждого своя причина:

- Класс **реализует цель** и берёт в конструкторе **ровно один** параметр её типа — оборачиваемый
  экземпляр. Остальные параметры резолвятся из скоупа, которому принадлежит регистрация, включая
  `@Named`. `@CobaltParam` и поля `@injected` отвергаются: декоратор применяет скоуп, места вызова
  нет.
- Двум декораторам одной регистрации нужен **`order:`**, меньший — внутренний. Сборка не угадывает
  и не принимает два одинаковых порядка. Декораторы, чьи окружения не пересекаются, не конкурируют.
- Цель должна быть **зарегистрирована везде, где декоратор активен**, или названа в `provides:`.
  Его зависимости проходят проверку полноты, как у любого класса, а декоратор, которому нужно
  что-то, зависящее от его собственной цели, — это цикл.
- Декоратор **не может брать ленивую async-регистрацию**: он выполняется синхронно, когда экземпляр
  отдают. Оборачивать ленивую можно.
- Async-класс, который во время фазы 1 резолвит декорированную регистрацию, **ждёт то, что резолвит
  декоратор**. Генератор дописывает это в его `dependsOn` — потребителю, а не цели, поэтому
  override цели ожидание не убирает.

В рантайме это тот же `decorate`, что в Manual Mode: удерживаемая регистрация декорируется один раз
и результат общий, override декорируется как регистрация, которую он заменил, а скоуп закрывает
внутренний экземпляр и никогда — декоратор.
...
`CobaltScopeWidget`, scoped-виджеты и каждый маршрут, владеющий флоу, тоже принимают `overrides` —
функцией, которая вызывается при каждом создании их скоупа. Так в виджет-тесте монтируют один экран
с двойником на месте. Они подменяют то, что регистрирует этот скоуп; ключ, которым владеет предок,
подменяется у предка, а попытка сделать это в дочернем скоупе падает с именем владельца.
