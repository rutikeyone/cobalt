...
Тот же список принимают `$startCobalt(overrides: [...])` и `CobaltAppScope(overrides: [...])` — в
приложении это флейвор или debug-меню. `.value` принимает готовый объект, `.lazy` и `.transient` —
