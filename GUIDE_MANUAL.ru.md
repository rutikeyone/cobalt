...
Декоратор оборачивает то, что отдаёт регистрация, не трогая её класс, — логирование, повторы, кэш,
метрика вокруг чужого клиента:
...
Декораторы применяются в порядке добавления, первый — самый внутренний, и видят резолвер скоупа,
которому принадлежит регистрация. Удерживаемая регистрация декорируется один раз, при первом резолве,
и результат общий для всех; transient и параметризованная — при каждой сборке. Где `decorate` стоит
относительно регистрации внутри `build()`, неважно, а override декорируется так же, как регистрация,
которую он заменил.

Скоуп по-прежнему владеет внутренним экземпляром и закрывает его один раз; декоратору закрывать
нечего. Отвергаются три ошибки: декорирование ключа, который уже резолвили (его держатели остались бы
с необёрнутым экземпляром), декорирование ключа, которого скоуп не регистрирует, — `runBuilder`
называет предка-владельца, — и декоратор, резолвящий собственный ключ, — это `CobaltCycleError`.
...
Чтобы он был готов до открытия экрана, прогрейте его. `warmUp` запускает все сборки из списка сразу,
не на пути старта; каждая — та же сборка, что запустил бы `getAsync`, поэтому экран, который попросит
в это время, дождётся её, а не начнёт вторую:
